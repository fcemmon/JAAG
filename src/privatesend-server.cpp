...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all JAAG Coin related functionality
...
/*
    ...
    allow endless transaction that would bloat JAAG Coin and make it unusable. To
...
{
    if(fLiteMode) return; // disable all JAAG Coin specific functionality
