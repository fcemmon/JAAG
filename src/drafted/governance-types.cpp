...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "JAAGNetwork") type = JAAGNetwork;
    if(strType == "JAAGNetworkVariable") type = JAAGNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == JAAGNetwork) s = "JAAGNetwork";
    if(type == JAAGNetworkVariable) s = "JAAGNetworkVariable";
