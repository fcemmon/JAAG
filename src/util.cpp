...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "jaag coin" is a composite category enabling all JAAG Coin-related debug output
            if(ptrCategory->count(string("jaag"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JAAGCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JAAGCoinCore
    // Mac: ~/Library/Application Support/JAAGCoinCore
    // Unix: ~/.jaagcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JAAGCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JAAGCoinCore";
#else
    // Unix
    return pathRet / ".jaagcoincore";
#endif
#endif
}
